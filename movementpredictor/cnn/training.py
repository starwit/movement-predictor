...
def train_model(path_data, path_model, architecture, output_prob, pixel_per_axis,
    ...
    log.info("train size: " + str(len(train_loader)))
    log.info("val size: " + str(len(val_loader)))
    ...
    for epoch in range(100):
      for count, (input, target, _, _) in tqdm(enumerate(train_loader)):
        ...
        if (count+1) % batch_interval == 0:  # check after 'batch_interval' batches
...
def store_parameters(history, config: ModelConfig):
   ...
   paras = {
      ...
      "obj_type_of_interest": config.class_of_interest
    }
